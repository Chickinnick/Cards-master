...
public abstract class ResizableView extends FrameLayout implements BaseTextView {
    ...
    private void init(Context context) {
        ...
        int sizeW = convertDpToPixel(SELF_SIZE_DP_W, getContext());
            ...
            if (view.getTag().equals("DraggableViewGroup")) {
                ...
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        this_orgX = ResizableView.this.getX();
                        this_orgY = ResizableView.this.getY();

                        scale_orgX = event.getRawX();
                        scale_orgY = event.getRawY();
                        scale_orgWidth = ResizableView.this.getLayoutParams().width;
                        scale_orgHeight = ResizableView.this.getLayoutParams().height;

                        rotate_orgX = event.getRawX();
                        rotate_orgY = event.getRawY();

                        centerX = ResizableView.this.getX() +
                                ((View) ResizableView.this.getParent()).getX() +
                                (float) ResizableView.this.getWidth() / 2;
                        ...
                        centerY = ResizableView.this.getY() +
                                ((View) ResizableView.this.getParent()).getY() +
                                statusBarHeight +
                                (float) ResizableView.this.getHeight() / 2;
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if (length2 > length1
                            ...
                            ResizableView.this.getLayoutParams().width += offset;
                            ResizableView.this.getLayoutParams().height += offset;
                            onScaling(true);
                            //DraggableViewGroup.this.setX((float) (getX() - offset / 2));
                            //DraggableViewGroup.this.setY((float) (getY() - offset / 2));
                        } else if (length2 < length1
                                && (angle_diff < 25 || Math.abs(angle_diff - 180) < 25)
                                && ResizableView.this.getLayoutParams().width > size / 2
                                && ResizableView.this.getLayoutParams().height > size / 2) {
                            //scale down
                            double offsetX = Math.abs(event.getRawX() - scale_orgX);
                            double offsetY = Math.abs(event.getRawY() - scale_orgY);
                            double offset = Math.max(offsetX, offsetY);
